...
test.describe("Image Swap Extension", () => {
    ...
    test("replaces static images with placeholders", async ({ page }) => {
        ...
        await expect(staticImage).not.toHaveAttribute("src", /^data:image\/png;base64/);
    ...
    test("replaces dynamically added images", async ({ page }) => {
        ...
        const lastWidth = await dynamicImage.getAttribute("data-800x600-last-width");
        const lastHeight = await dynamicImage.getAttribute("data-800x600-last-height");
    ...
    test("placeholder contains correct dimensions", async ({ page }) => {
        ...
        const lastHeight = await imgElement.getAttribute("data-800x600-last-height");
    ...
    test("replaces images with srcset and preserves original srcset", async ({
        ...
        const lastHeight = await srcsetImage.getAttribute("data-800x600-last-height");
    ...
    test("handles dynamically added images with srcset", async ({ page }) => {
        ...
        // Verify it masked
        const styleContent = await dynamicSrcsetImage.evaluate(el => el.style.content);
        expect(styleContent).toMatch(/^url\("data:image\/png;base64/);
        ...
        const lastWidth = await dynamicSrcsetImage.getAttribute("data-800x600-last-width");
        const lastHeight = await dynamicSrcsetImage.getAttribute("data-800x600-last-height");
    ...
    test("handles picture elements with source srcsets", async ({ page }) => {
        ...
        const lastWidth = await pictureImage.getAttribute("data-800x600-last-width");
        const lastHeight = await pictureImage.getAttribute("data-800x600-last-height");
